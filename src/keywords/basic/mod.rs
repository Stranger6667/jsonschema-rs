...
use crate::{compilation::CompilationContext, error::CompilationError, keywords::BoxedValidator};
use serde_json::{Map, Value};
use std::collections::HashSet;

pub(crate) fn compile_basic_validators(
    validators: &mut Vec<BoxedValidator>,
    unprocessed_keywords: &HashSet<String>,
    context: &CompilationContext,
    object: &Map<String, Value>,
) -> Result<(), CompilationError> {
    for (keyword, subschema) in object {
        // If the keyword has not been consumed by the creation of combined validator
        if unprocessed_keywords.contains(keyword) {
